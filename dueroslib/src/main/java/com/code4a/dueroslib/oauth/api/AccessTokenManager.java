...
public class AccessTokenManager implements Parcelable {
    ...
    private void compareWithConfig() {
         ...
         */
        final SharedPreferences sp = this.context.getSharedPreferences(OauthPreferenceUtil.BAIDU_OAUTH_CONFIG, Context.MODE_PRIVATE);
     ...
     * @param values token信息的key-value形式
     */
    protected void storeToken(OauthClientCredentialsInfo values) {
        if (values == null) {
            return;
        }
        this.accessToken = values.getAccess_token();
        // expires_in 返回值为秒
        long expiresIn = values.getExpires_in() * 1000;
        this.expireTime = System.currentTimeMillis() + expiresIn;
        Log.e("Oauth", "expiresIn : " + expiresIn + " ,expireTime : " + expireTime);
        HttpConfig.setAccessToken(this.accessToken);
        OauthPreferenceUtil.setAccessToken(context, this.accessToken);
        OauthPreferenceUtil.setCreateTime(context, System.currentTimeMillis());
        OauthPreferenceUtil.setExpires(context, expiresIn);
    }
    ...
    protected boolean isSessionValid() {
        if (this.accessToken == null || this.expireTime == 0) {
            Log.e("isSessionValid", "accessToken is null or expireTime = 0");
            initToken();
        }
