 ...
 * TODO: 百度Oauth2授权方式完善
 */
public class BaiduOauthImplicitGrant implements Parcelable, BaiduOauth {
     ...
     * @param configs   configs[0]是否强制登录，如果该参数为true的话，会强制用户进行登录。
     *                  configs[0]是否确认登录，如果该参数为true的话，如果用户当前为登录状态，
    ...
    public void authorize(Application activity, BaiduOauthListener listener, boolean ...configs) {
        this.authorize(activity, null, configs[0], configs[1], (BaiduDialog.BaiduDialogListener) listener);
    }
     ...
     */
    private void authorize(Application activity,
                           String[] permissions,
